...
export class SeaComponent implements OnInit {
  ...
  onPostSubmit() {
    if (this.postsForm.valid) {
      if (this.editPostsMode) {
        this.postservice.updatePostsSeas(this.postsForm.value).subscribe(
          (res) => {
            this.getPosts();
        ...
        this.postservice.addpostsSeas(this.postsForm.value).subscribe(
          (res: any) => {
            this.getPosts();
            this.onClosePostsModal();
            this.onReset();
  ...
  getPosts() {
    this.postservice.getPostsListSeas().subscribe((res: Posts[]) => {
      console.log(res);
      this.posts = res;
    });
  }
  ...
  onDeletePosts(id: any) {
    if (confirm('Are you sure you want to delete this post?')) {
      this.postservice.deletePostsSeas(id).subscribe(
        (res) => {
          console.log(res);
          this.getPosts();
        },
